# physics_md_al_fe — MD-моделирование деформации Al-матрицы с включением Fe₄Al₁₃ при эквивалентном магнитострикционном механическом воздействии

## 1. Название и суть

Проект посвящён молекулярно-динамическому (MD) моделированию алюминиевой матрицы
с интерметаллидным включением Fe₄Al₁₃ (Al₁₃Fe₄), к которому приложен механический
эквивалент магнитострикционного воздействия (собственная деформация включения). Цель —
выяснить, при каком масштабе модели и уровне воздействия в алюминии у включения
зарождаются подтверждённые дефекты и дислокации.

## 2. Цель проекта

Построить атомистическую модель «матрица Al + включение Fe₄Al₁₃» и исследовать, как
локальное напряжение от эквивалентной (механически заданной) деформации включения
влияет на:
...
| **Stage C 1M** | масштабный расчёт ≈ 944 812 атомов, eps_z = 0.0100 | **запущен, идёт счёт; результаты — отдельным отчётом, в выводах не используются** |
...
Stage C в физических выводах настоящего отчёта не используются и будут оформлены
...
- **Отчёты**: `docs/reports/stageB_focused_100k_and_stageC_1M_report.(md|tex|pdf)`.
